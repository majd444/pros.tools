...
const PluginsTab: React.FC<PluginsTabProps> = ({ plugins, getAgentId }) => {
  ...
  // Discord config dialog state
  const [showDiscordConfig, setShowDiscordConfig] = React.useState(false)
  const [discordClientId, setDiscordClientId] = React.useState("")
  const [discordBotToken, setDiscordBotToken] = React.useState("")
  const [discordSaving, setDiscordSaving] = React.useState(false)
  ...
  const handleClick = async (pluginId: string) => {
    ...
    if (pluginId === "discord") {
      ...
      try {
        ...
        console.error('Failed to get agent id for discord dialog', e)
      }
    }
  }
  ...
  return (
    <TabsContent value="plugins" className="space-y-6">
      ...
      <div className="mt-6">
        <h3 className="text-lg font-medium mb-4 text-black">Add Plugin</h3>
        <div className="bg-white border rounded-lg p-4 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input type="password" value={discordBotToken} onChange={(e) => setDiscordBotToken(e.target.value)} placeholder="Bot token" className="text-black" />
            </div>
          </div>
          {discordError && (
            ...
            <Button
              onClick={async () => {
                try {
                  ...
                  setDiscordError(null)
                  const id = discordAgentId || await getAgentId()
                  setDiscordAgentId(id)
                  if (!discordClientId.trim() || !discordBotToken.trim()) {
                    setDiscordError('Client ID and Bot Token are required')
                    return
                  }
                  await saveDiscordConfig({
                    agentId: id as unknown as Id<'agents'>,
      ...
      <Dialog open={showDiscordConfig} onOpenChange={(open) => { setShowDiscordConfig(open); if (!open) { setDiscordError(null); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Discord Integration</DialogTitle>
            <DialogDescription>
              Add this code to your website to integrate the Discord plugin.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {discordError && (
              <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">{discordError}</div>
            )}
            <div className="pt-2">
              <Button asChild variant="outline">
                <a href="https://discord.com/developers/applications" target="_blank" rel="noreferrer">Open Discord Developer Portal</a>
              </Button>
            </div>
            <div className="space-y-2">
              <Label htmlFor="discord-agent-id" className="text-black">Agent ID</Label>
              <Input id="discord-agent-id" value={discordAgentId} readOnly />
            </div>
            <div className="space-y-2">
              <Label htmlFor="discord-client-id" className="text-black">Discord Client ID</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="discord-bot-token" className="text-black">Bot Token</Label>
              <Input
            ...
            <div className="flex gap-2 justify-end pt-2">
              <Button variant="secondary" onClick={() => setShowDiscordConfig(false)} disabled={discordSaving}>Cancel</Button>
              <Button
                onClick={async () => {
                  try {
                    setDiscordSaving(true)
                    setDiscordError(null)
                    const agentId = discordAgentId || await getAgentId()
                    if (!discordClientId.trim() || !discordBotToken.trim()) {
                      setDiscordError('Client ID and Bot Token are required')
                      return
                    }
                    await saveDiscordConfig({
                      agentId: agentId as unknown as Id<'agents'>,
                      clientId: discordClientId.trim(),
                      token: discordBotToken.trim(),
                    })
